...
def feedForward(x, params,B):
    x = layers.quantizeAct(x,B+2)
    l=0
    current_params = params[l]
    current_params[0] = layers.quantizeWeight(current_params[0],B+5)
    current_params[1] = layers.quantizeWeight(current_params[1],B+5)
    c1 = layers.linOutermost(x,current_params)
    c1 = layers.slopedClipping(c1)
    c1 = layers.quantizeAct(c1,B+3)

    l+=1
    current_params = params[l]
    current_params[0] = layers.quantizeWeight(current_params[0],B+3)
    current_params[1] = layers.quantizeWeight(current_params[1],B+3)
    c2 = layers.linOutermost(c1,current_params)
    c2 = layers.slopedClipping(c2)
    c2 = layers.quantizeAct(c2,B+2)
